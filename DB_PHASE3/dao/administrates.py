...
class AdministratesDAO:
    ...
    def insert(self, users_id, group_id):
        cursor = self.conn.cursor()
        query = "insert into administrates(users_id,group_id) values(%s,%s) returning users_id;"
    ...
    def remove(self, users_id, group_id):
        cursor = self.conn.cursor()
        query = "DELETE from administrates where users_id = %s and group_id = %s returning users_id;"
        cursor.execute(query, (users_id, group_id,))
        users_id = cursor.fetchone()[0]
        self.conn.commit()
        return users_id