...
class MemberDAO:
    ...
    def getMembershipsByUserName(self, first_name, last_name):
        cursor = self.conn.cursor()
        query = "select * from member_of;"
        cursor.execute(query)
        print(cursor)
        query = "select * from member_of natural inner join users natural inner join group_chat where first_name = %s and last_name = %s;"
