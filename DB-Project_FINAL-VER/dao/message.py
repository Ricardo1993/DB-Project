...
class MessageDAO:
    ...
    def searchMessagesByGroupId2(self, group_id):
        cursor = self.conn.cursor()
        query = "select Message.message_id, message.message_text, Message.message_date, Users.first_name, Users.last_name " \
                "from message natural inner join messages natural inner join users natural inner join sent where messages.group_id = %s" \
                " order by Message.message_id desc;"
        # query = "select * from Message"
        print('Huelga 2018')
